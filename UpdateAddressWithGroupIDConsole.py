...
def update_im_carrier_kpn():
    ...
    if existsInImCP=='Y':
        ...
        if carrier_id == '':
            print('Carrier_ID was not updated from im_carrier_kpn for Address to Configure, Program will add new carrier_id with cp_id derived, cp_id: '+str(im_cp_id_created))
...
def insert_im_carrier_kpn():
    print('In insert_im_carrier_kpn() safasc')
    global old_addressID, im_cp_id_created, carrier_id, group_id, existsInImCP
    print('In insert_im_carrier_kpn()')
    fetchNextCarrierID()
    if carrier_type=='Fiber':
